...
import com.royalcommission.bs.modules.api.model.PathologyResponse;
...
import com.royalcommission.bs.views.adapters.PathologyAdapter;
...
public class PathologyFragment extends BaseFragment implements PathologyAdapter.TestResultListener{
    ...
    public static PathologyFragment getInstance(String appointmentId, boolean isCalendar) {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, ViewGroup container,
        ...
        View view = inflater.inflate(R.layout.fragment_pathology, container, false);
        initView(view);
        return view;
    }
    ...
    private void initView(View view) {
        if (view != null) {
            ...
            View.OnClickListener onClickListener = viewIP -> {
                if (viewIP.getId() == R.id.three_month) {
                    ...
                    if (isValidString(startDate)
                        ...
                        getPathology(SharedPreferenceUtils.getPatientID(AppController.getInstance()), SharedPreferenceUtils.getHospitalID(AppController.getInstance()), startDate, endDate);
                    } else {
            ...
            RecyclerView pathologyRecyclerView = view.findViewById(R.id.pathology_recycler_view);
            setupPathologyAdapter(pathologyRecyclerView);
    ...
    private void getPathology(String patientID, String hospitalID, String startDate, String endDate) {

        processRequest(AppController.getWebService().getPathology(patientID, hospitalID, startDate, endDate), false, true, null, new RetrofitListener() {
            @Override
            public void onSuccess(Object object) {
                if (object != null) {
                    PathologyResponse pathologyResponse = RetrofitResponseParser.convertInstanceOfObject(object, PathologyResponse.class);
                    if (pathologyResponse != null) {
                        BaseResponse baseResponse = pathologyResponse.getBaseResponse();
                        if (baseResponse != null) {
                            if (baseResponse.getResponseCode() == 1) {
                                List<Pathology> list = pathologyResponse.getPathologyList();
                                if (list != null) {
                                    pathologyList.clear();
                                    if (!list.isEmpty())
        ...
        }, PathologyResponse.class);

    }

    private void setupPathologyAdapter(RecyclerView pathologyRecyclerView) {
        if (pathologyRecyclerView != null) {
            pathologyAdapter = new PathologyAdapter(getActivity(), pathologyList, this);
            RecyclerView.LayoutManager mPathologyAdapterLayoutManager = new LinearLayoutManager(getActivity());
            pathologyRecyclerView.setLayoutManager(mPathologyAdapterLayoutManager);
            pathologyRecyclerView.setItemAnimator(new DefaultItemAnimator());
            pathologyRecyclerView.setHasFixedSize(true);
            pathologyRecyclerView.setNestedScrollingEnabled(false);
            pathologyRecyclerView.setAdapter(pathologyAdapter);
        }
    }
