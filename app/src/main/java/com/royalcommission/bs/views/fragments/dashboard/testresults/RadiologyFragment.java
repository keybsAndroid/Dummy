...
import com.royalcommission.bs.modules.api.model.ScanResultResponse;
...
import com.royalcommission.bs.views.adapters.RadiologyExpandableAdapter;
 ...
 *
 */
public class RadiologyFragment extends BaseFragment implements TestResultListener {

    private List<ScanResult> scanResultList = new ArrayList<>();
    private RadiologyExpandableAdapter radiologyExpandableAdapter;
    ...
    public static RadiologyFragment getInstance(boolean isCalendar) {
    ...
    private void initView(View view) {
        if (view != null) {
            ...
            View.OnClickListener onClickListener = viewIP -> {
                if (viewIP.getId() == R.id.three_month) {
                    ...
                    if (isValidString(startDate)
                        ...
                        getRadiologyByDate(SharedPreferenceUtils.getPatientID(AppController.getInstance()), SharedPreferenceUtils.getHospitalID(AppController.getInstance()), startDate, endDate);
                    } else {
            ...
            RecyclerView radiologyRecyclerView = view.findViewById(R.id.expandable_list_view);
            setupRadiologyExpandableAdapter(radiologyRecyclerView);
    ...
    private void getRadiologyByDate(String patientID, String hospitalID, String startDate, String endDate) {

        processRequest(AppController.getWebService().getRadiologyByDate(patientID, hospitalID, startDate, endDate), false, true, null, new RetrofitListener() {
            @Override
            public void onSuccess(Object object) {
                if (object != null) {
                    ScanResultResponse scanResultResponse = RetrofitResponseParser.convertInstanceOfObject(object, ScanResultResponse.class);
                    if (scanResultResponse != null) {
                        BaseResponse baseResponse = scanResultResponse.getBaseResponse();
                        if (baseResponse != null) {
                            if (baseResponse.getResponseCode() == 1) {
                                List<ScanResult> list = scanResultResponse.getScanResultList();
                                if (list != null) {
                                    scanResultList.clear();
                                    if (!list.isEmpty())
                                        scanResultList.addAll(list);
                                    if (radiologyExpandableAdapter != null)
    ...
    private void setupRadiologyExpandableAdapter(RecyclerView radiologyRecyclerView) {
        if (radiologyRecyclerView != null) {
            radiologyExpandableAdapter = new RadiologyExpandableAdapter(getActivity(), scanResultList, this);
            RecyclerView.LayoutManager mRadiologyAdapterLayoutManager = new LinearLayoutManager(getActivity());
            radiologyRecyclerView.setLayoutManager(mRadiologyAdapterLayoutManager);
            radiologyRecyclerView.setItemAnimator(new DefaultItemAnimator());
            radiologyRecyclerView.setNestedScrollingEnabled(false);
            radiologyRecyclerView.setHasFixedSize(true);
            radiologyRecyclerView.setAdapter(radiologyExpandableAdapter);
        }
    }
    ...
    private void goToGetDetails(String orderID) {
        if (isValidString(orderID)) {
            addFragment(TestResultsDetailFragment.newInstance(true, orderID, "", ""), true);
        }
    }

    @Override
    public void onDetails(String orderID) {
        goToGetDetails(orderID);
    }
}
