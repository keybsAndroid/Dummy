...
public class MealsFragment extends BaseFragment implements MealsAdapter.MealsClickListener {
    ...
    public static MealsFragment getInstance(int position) {
        // Required empty public constructor
        selection = position;
    ...
    public View onCreateView(@NotNull LayoutInflater inflater, ViewGroup container,
        ...
        if (view == null) {
            ...
            View patientCareGiver = view.findViewById(R.id.patient_caregiver_layout);
            if (todayTaskView != null) {
                ...
                if (selection == 1) {
                    patientCareGiver.setVisibility(View.GONE);
                    todayTaskView.setVisibility(View.VISIBLE);
                } else {
                    patientCareGiver.setVisibility(View.VISIBLE);
                    todayTaskView.setVisibility(View.GONE);
                }
            }
    ...
    private void setAdapter(RecyclerView recyclerView) {
        ...
        recyclerView.setLayoutManager(new GridLayoutManager(getActivity(), Constants.NUMBER_OF_MEALS_COLUMNS));
        recyclerView.addItemDecoration(new GridSpacingItemDecoration(Constants.NUMBER_OF_MEALS_COLUMNS, CommonUtils.dpToPx(recyclerView.getContext(), valueInPixels), true));
                    ...
                    if (baseResponse != null) {
                        if (baseResponse.getResponseCode() == 1) {
                            ...
                            if (todayMealList != null && !todayMealList.isEmpty()) {
                                Collections.sort(todayMealList);
    ...
    public void onClick(String title, TodayMeal todayMeal) {
        if (todayMeal != null) {
            if (getActivity() != null) {
                ((HomeActivity) getActivity()).loadFragments(MealsDetailFragment.getInstance(selection, title, todayMeal), true, MealsDetailFragment.TAG);
            }
        }
    }
}
